...
function FreshOrders() {
    ...
    async function _fetchAssignments() {
        try {
            ...
            console.log(data , 'fresh order data')
            assignmentList = [];
            if (data.length !== 0) {
                for (let index = 0; index < data.length; index++) {
                    assignmentList.push({
    ...
    async function fetchExperts(subject,input) {
                ...
                <ModalContent maxH={'500px'} overflowY='scroll'>
                    ...
                    <ModalFooter>
                        <Button onClick={async () => {
                            ...
                            try {
                                console.log(config);
                                const response = await axios.post(apiUrl + '/expert/assignment/ask/doable',
                                    {
    ...
    async function openDoableResponsesModal(index) {
        ...
        try {
            ...
            const response = await axios.get(apiUrl + '/assignment/doability/fetch?assignment_id=' + assignments[index].id, config
            );
                ...
                <ModalContent>
                    ...
                    <ModalBody>
                        <Table variant='simple' size="sm">
                            ...
                            <Tbody>
                                {(doableResponses.length === 0) ? <></> : doableResponses.map(
                                    (doableResponse, index) => <Tr key={index}>
    ...
    async function _fetchQuoteAssignmentDetail(index) {
        setQuoteAssignmentData({
            ...
            deadline: new Date(assignments[index].deadline).toLocaleTimeString() + ", " + new Date(assignments[index].deadline).toDateString()
        })
    }
    ...
    function QuotationModal() {
        ...
        return (
            ...
            >
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader id="assId">{(assignments.length === 0) ? "" : quoteAssignmentData._id}</ModalHeader>
                    <ModalCloseButton />
                    <ModalBody>
                        <HStack padding={2}>
                            <Text fontWeight={'bold'}>Student ID:</Text>
                            <Text>{(assignments.length === 0) ? "" : quoteAssignmentData.client_id}</Text>
                        </HStack>
                        <HStack padding={2}>
                            <Text fontWeight={'bold'}>Subject:</Text>
                            <Text>{(assignments.length === 0) ? "" : quoteAssignmentData.subject}</Text>
                        </HStack>
                        <VStack padding={2} alignItems={'left'}>
                            <Text fontWeight={'bold'}>Description:</Text>
                            <Textarea contentEditable={false} onChange={ ()=>{ }} value={(assignments.length === 0  ) ? "" : (description==" ")? quoteAssignmentData.description: description}>{quoteAssignmentData.description}</Textarea>
                        ...
                        <HStack padding={2}>
                            ...
                            <Text>{(assignments.length === 0) ? "" : quoteAssignmentData.level}</Text>
                        </HStack>
                        <HStack padding={2}>
                            <Text fontWeight={'bold'}>Reference:</Text>
                            <Text>{(assignments.length === 0) ? "" : quoteAssignmentData.reference}</Text>
                        </HStack>
                        <HStack padding={2}>
                            <Text fontWeight={'bold'}>Pages:</Text>
                            <Text>{(assignments.length === 0) ? "" : quoteAssignmentData.numOfPages}</Text>
                        </HStack>
                        <VStack padding={2} alignItems={'left'}>
                        ...
                        <Checkbox isChecked={isCustom} value={isCustom} onChange={(e) => { setIsCustom(e.target.checked) }} padding={2} spacing={'5'} fontWeight={'bold'}>Set Custom CP1 Value</Checkbox>
                        <FormControl display={(isCustom && !isExternal) ? "block" : "none"} h={'full'} padding={2} id="cp1">
                        ...
                        <Checkbox isChecked={isExternal} value={isExternal} onChange={(e) => { setExternal(e.target.checked) }} padding={2} spacing={'5'} fontWeight={'bold'}>External Payment</Checkbox>
                        <InputGroup>
                            <Input type='file' id='addFileStickyNotes' ref={inputFileStickyNotes}
                                onChange={() => {
                                    let fileUrl = '';
                                                ...
                                                .then(async function (response) {
                                                    ...
                                                    if (fileUrl === "") {
                                                        ...
                                                        if (response.data.success) {
                                                            ...
                                                            window.alert(`${inputFileStickyNotes.current.files[0].name} has been added to sticky notes, fill other details and submit`);
                                                        }
                                                        else {
    ...
    return (
        <>
                                    ...
                                    <HStack>
                                        {
                                            (assignment.status === 'Fresh Order') ? <>
                                                <Button
                                                    ...
                                                    onClick={async () => openModal(index)}>Give Quote</Button>
                                                <Button
                                                    display={(localStorage.getItem('userRole') === "Operator" || localStorage.getItem("userRole") === "Super Admin" || localStorage.getItem("userRole") === "Admin") ? 'flex' : 'none'}
                                                    onClick={async () => openExpertModal(index)}>Ask Quote Expert</Button>
                                                <Button
                                                    display={(localStorage.getItem('userRole') === "Operator" || localStorage.getItem("userRole") === "Super Admin" || localStorage.getItem("userRole") === "Admin") ? 'flex' : 'none'}
                                                    onClick={async () => openAskDoableModal(index)}>Ask Doable Expert</Button>
                                                ...
                                                <Button
                                                    display={(localStorage.getItem('userRole') === "Operator" || localStorage.getItem("userRole") === "Super Admin" || localStorage.getItem("userRole") === "Admin") ? 'flex' : 'none'}
                                                    onClick={async () => openQuotesModal(index)}>Quotation Responses</Button>
                                                <Button
                                                    display={(localStorage.getItem('userRole') === "Operator" || localStorage.getItem("userRole") === "Super Admin" || localStorage.getItem("userRole") === "Admin") ? 'flex' : 'none'}
                                                    onClick={async () => openAskDoableModal(index)}>Ask Doable Expert</Button>
                                            </> : <>
                                                ...
                                                <Button
                                                    display={(localStorage.getItem('userRole') === "Operator" || localStorage.getItem("userRole") === "Super Admin" || localStorage.getItem("userRole") === "Admin") ? 'flex' : 'none'}
                                                    onClick={async () => openDoableResponsesModal(index)}>Doable Responses</Button>
                                            </>
                                        }
                                    </HStack>
                                </Td>
            ...
            <div className="ShowSideClick checkk" display={{ base: 'block', sm: 'none', md: 'none' }}>
                {(assignments.length === 0) ? <></> : assignments.map(

                    (assignment, index) =>
                        <Accordion defaultIndex={[0]} allowMultiple className="p-set " display={{ base: 'block', sm: 'none', md: 'none' }}>
                            <AccordionItem>
                                <h2>
                                    <AccordionButton>
